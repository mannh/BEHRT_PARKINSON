...
class NextVisit(Dataset):
    def __init__(self, token2idx, label2idx, age2idx, dataframe, max_len, code='code', age='age', label='label',
                 patid='patid', creatinine='Creatinine'):
        ...
        self.creatinine = dataframe[creatinine]
    ...
    def __getitem__(self, index):
        ...
        creatinine = self.creatinine[index]

        # extract data
        age = age[(-self.max_len + 1):]
        code = code[(-self.max_len + 1):]
        creatinine = creatinine[(-self.max_len + 1):]
        ...
        # squeeze the ALS FRS functions to range of 0-1
        label = np.array([float(x.split('_')[-1]) / 4 for x in label], dtype='float32')
        ...
        creatinine = seq_padding(creatinine, self.max_len, symbol=-1)

        return torch.LongTensor(age), torch.LongTensor(code), torch.LongTensor(position), torch.FloatTensor(
            creatinine), torch.LongTensor(segment), \
